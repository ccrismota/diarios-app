...
export class DiarioListComponent implements OnInit {
        ...
        if (result) {
          ...
          this.diariosService
            ...
            .pipe(
              this.toast.observe({
                loading: 'Adicionando...',
                error: 'Ocoreu um erro',
                success: 'Diario Adicionado',
              })
            )
          .subscribe();
        }
  ...
  onClickEdit(diario: Diario){
    const ref = this.dialog.open(DiarioEditComponent, {maxWidth: '512px', data: {...diario}
  });
    ref.afterClosed().subscribe({
      next: (result) => {
        if(result){
  ...
  onClickDelete(diario: Diario) {
    ...
    if (canDelete) {
      this.diariosService
        .deleteDiario(diario)
        .pipe(this.toast.observe({ success: 'Diario Apagado!' }))
        .subscribe();
    }
  }
